...
# ── ultralytics 缓存目录:把自动下载的权重(含 AMP 校验模型)落到指定盘,不散落到 C 盘 ──
# 默认指向用户要求的 E 盘缓存目录;可用环境变量 SEHUNTER_CACHE_DIR 覆盖。
# 注意:必须在 `import ultralytics` 之前调用 _setup_ultralytics_cache(),
# 因为 ultralytics 在 import 时就把 settings 读进内存了。
_DEFAULT_CACHE_DIR = r"E:\2026C4\Drone\2\2026\cache"


def _cache_dir():
    import os
    return os.environ.get("SEHUNTER_CACHE_DIR", "").strip() or _DEFAULT_CACHE_DIR


# 已知的官方权重名(用于判断起始权重是不是"裸名官方权重",从而锁定到缓存目录)。
# 不在此列表也没关系:只要看起来像 yolo*.pt 的裸文件名,都按官方权重处理。
def _looks_like_official_weight(s):
    import os
    s = str(s).strip()
    if os.sep in s or "/" in s or os.path.isabs(s):
        return False                      # 带路径的,用户自己指定了位置,不动
    base = os.path.basename(s).lower()
    return base.endswith(".pt") and (base.startswith("yolo") or base.startswith("rtdetr"))


def _ensure_official_weight(name):
    """把官方权重(裸名,如 'yolo26n.pt')解析成缓存目录里的绝对路径。
    - 缓存里已有 → 直接返回该路径(不下载);
    - 缓存里没有 → 下载到缓存目录后返回该路径(只这一次,以后常驻);
    解析/下载失败 → 原样返回 name,交给 ultralytics 默认逻辑兜底。"""
    import os
    cdir = _cache_dir()
    weights = os.path.join(cdir, "weights")
    target = os.path.join(weights, os.path.basename(name))
    try:
        os.makedirs(weights, exist_ok=True)
        if os.path.exists(target):
            print(f"[train] 起始权重命中缓存: {target}(不下载)", flush=True)
            return target
        # 缓存没有,下载到缓存目录
        from ultralytics.utils.downloads import attempt_download_asset
        print(f"[train] 缓存缺 {os.path.basename(name)},下载到 {target} ...", flush=True)
        got = attempt_download_asset(target)   # 显式给缓存内绝对路径,下到这里
        got = str(got) if got else target
        # 个别版本可能下到当前目录,挪进缓存,保证以后命中
        if os.path.exists(got) and os.path.abspath(got) != os.path.abspath(target):
            try:
                import shutil
                ...
                pass
        if os.path.exists(target):
            print(f"[train] 已缓存起始权重: {target}(以后不再下载)", flush=True)
            return target
        return got
    except Exception as e:
        print(f"[train] 准备起始权重失败(忽略,交给默认逻辑): {e}", flush=True)
        return name


def _setup_ultralytics_cache():
    """把 ultralytics 的配置目录与权重/数据/运行目录都指到统一缓存目录。
    在导入 ultralytics 之前调用最稳妥;失败只警告,不阻断训练。"""
    import os
    cdir = _cache_dir()
    try:
        ...
        weights = os.path.join(cdir, "weights")
        os.makedirs(weights, exist_ok=True)
        # YOLO_CONFIG_DIR:ultralytics 配置(settings.json 等)目录,import 前生效
        os.environ.setdefault("YOLO_CONFIG_DIR", cdir)
    except Exception as e:
        print(f"[train] 准备缓存目录失败(忽略): {e}", flush=True)
        return
    # 把 settings 里的 weights_dir / datasets_dir / runs_dir 也写到缓存目录,
    # 让自动下载的权重真正落到这里(settings 优先级高于默认相对路径)。
    try:
        from ultralytics.utils import SETTINGS
        updates = {"weights_dir": weights}
        try:
            SETTINGS.update(**updates)   # 新版 SettingsManager 支持 update()
        except Exception:
            for k, v in updates.items():
                try:
                    ...
                    pass
        print(f"[train] ultralytics 缓存目录 -> {cdir}", flush=True)
    except Exception as e:
        print(f"[train] 写 ultralytics settings 失败(忽略): {e}", flush=True)


# AMP 检查会硬编码加载一个基准模型(新版是 yolo26n.pt)做 FP32/FP16 数值比对。
# 它和续训的模型无关,但若本地没有就会每次联网重下,网慢时卡在
# 'running AMP checks...' 很久。这里在训练前把它预置到上面那个统一缓存目录,
# 让 check_amp 直接命中本地、秒过、不再下载。
_AMP_CHECK_MODELS = ("yolo26n.pt", "yolo11n.pt", "yolov8n.pt")  # 不同版本 AMP 检查用过的基准


def _prepare_amp_check_model():
    import os
    cdir = _cache_dir()
    weights = os.path.join(cdir, "weights")
    # 候选查找目录:统一缓存目录 + 其 weights 子目录 + ultralytics 权重目录 + 当前工作目录
    wdir = None
    try:
        from ultralytics.utils import SETTINGS
        wdir = SETTINGS.get("weights_dir") if hasattr(SETTINGS, "get") else None
    except Exception:
        wdir = None
    search_dirs = [d for d in (weights, cdir, wdir, os.getcwd()) if d]
    have = None
    for d in search_dirs:
        for name in _AMP_CHECK_MODELS:
        ...
        if have:
            break
    if have:
        print(f"[train] AMP 校验模型已在本地: {have}(跳过下载)", flush=True)
        return
    # 本地都没有:主动下载一次到统一缓存目录的 weights 下(只这一次,之后命中本地)
    try:
        from ultralytics.utils.downloads import attempt_download_asset
        target = os.path.join(weights, _AMP_CHECK_MODELS[0])  # yolo26n.pt
        os.makedirs(weights, exist_ok=True)
        print(f"[train] 首次预置 AMP 校验模型 -> {target} ...", flush=True)
        attempt_download_asset(target)
        print("[train] AMP 校验模型已缓存,以后不再重复下载", flush=True)
    except Exception as e:
        # 失败也不阻断训练:大不了这次仍走 AMP 的默认下载逻辑
        print(f"[train] 预置 AMP 校验模型失败(忽略,不影响训练): {e}", flush=True)
...
def train_once(dataset_dir, base=None, epochs=50, imgsz=640, batch=16,
    ...
    from_base: 为 True 时强制用 base 作为起始权重(忽略 current),用于"指定基准训练对比";
               base 指向 .yaml(如 yolo11n.yaml)即从零训练(不加载预训练权重)。
    ...
    _prepare_amp_check_model()   # 训练前预置 AMP 校验模型,避免卡在重复下载

    data_yaml = _data_yaml(dataset_dir)            # 冻结验证集来自这里,评测始终用它
    reg = ModelRegistry(dataset_dir, promote_margin=promote_margin)

    if from_base and base:
        # 指定基准训练:无视 current,直接从所选基准开始(对比实验用)。
        # base 为 .yaml(如 yolo11n.yaml)时即从零训练;为 .pt 时从该权重开始。
        start_from = base
        parent_id = None
        print(f"[train] 指定基准训练(忽略 current): {start_from}")
    else:
        start_from = reg.current_ckpt() or base or "yolo11n.pt"
        parent_id = reg.current_id  # 记录这次是从哪个版本继续训的
    # 若起始权重是官方裸名(yolo26n.pt 等),锁定到缓存目录:有就用、没有下到缓存,绝不重下
    if _looks_like_official_weight(start_from):
        start_from = _ensure_official_weight(start_from)
...
def cmd_resume(args):
    ...
    if args.continue_training and osp.exists(last):
        ...
        _prepare_amp_check_model()   # 续训同样会触发 AMP 检查,先预置校验模型
...
def build_parser():
    ...
    t.add_argument("--from-base", action="store_true",
                   help="强制用 --base 作为起始权重,忽略注册表当前模型(对比实验/从零训练用)")
...
def main(argv=None):
    ...
    # 在任何 ultralytics 导入/训练之前,先把缓存目录指到统一位置(权重不落 C 盘)
    _setup_ultralytics_cache()
