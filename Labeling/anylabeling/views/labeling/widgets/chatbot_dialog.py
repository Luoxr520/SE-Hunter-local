...
class ChatbotDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.chat_history = []
        # 当前对话绑定的图片:用过一次 @image 后记住它,后续消息自动带上(无需再输 @image);
        # 切换图片时会更新为新图,确保后台分析的确实是切换后的图片。
        ...
        # 打开即载入当前图片:填预览、载入该图对话历史、预填 @image。
        # 用定时器延后到事件循环,确保预览控件已有有效尺寸,图片能正确缩放显示。
    ...
    def load_initial_data(self):
        ...
        if self.parent().filename:
            ...
            # 打开即默认针对当前主界面图片:输入框预填 "@image "(光标在其后),
            # 这样无需手动输入 @image 就能直接对当前图提问。
            # 仅在没有历史对话、且输入框为空时预填,避免覆盖已恢复的会话或用户输入。
            if not had_history and not self.message_input.toPlainText().strip():
                self.message_input.setPlainText("@image ")
                cursor = self.message_input.textCursor()
    ...
    def get_annotation_file_path(self, image_path):
        """Return the same-name X-AnyLabeling JSON file path for an image."""
        if not image_path:
            return None
    ...
    def load_annotation_context(self, image_path, max_shapes=80):
        """
        Read the current image's same-name X-AnyLabeling JSON file and convert
        ...
        lines = []
        lines.append("以下是当前图片对应的 X-AnyLabeling/YOLO 标注结果。")
    ...
    def build_message_text_with_annotation_context(self, message_text, image_path):
        ...
        if not annotation_context:
            annotation_context = (
                "当前图片没有找到同名 X-AnyLabeling JSON 标注文件。"
                "请仅根据图片独立判断；只有在目标清晰、位置能够较准确定位时，才输出待复核标注候选。"
            )

        return (
            f"{message_text}\n\n"
            "【当前 YOLO/X-AnyLabeling 标注上下文】\n"
            ...
            "```json\n"
            "{\n"
            "  \"xanylabeling_review_candidates\": [\n"
            "    {\n"
    ...
    def strip_review_candidates_json(self, text):
        ...
        if not text or "xanylabeling_review_candidates" not in text:
            return text

        def remove_if_review_block(match):
            block = match.group(0)
            return "" if "xanylabeling_review_candidates" in block else block
    ...
    def extract_review_candidates_from_response(self, text):
        ...
        for match in re.finditer(
            ...
            if "xanylabeling_review_candidates" in block:
                json_candidates.append(block)

        loose_json = self.extract_json_object_near_key(
            text, "xanylabeling_review_candidates"
        )
        if loose_json:
        ...
        for json_text in json_candidates:
            ...
            candidates = data.get("xanylabeling_review_candidates", [])
    ...
    def candidate_to_review_shape(self, candidate, index, image_width, image_height):
        """Convert one parsed model candidate into an X-AnyLabeling rectangle shape."""
    ...
    def apply_review_candidates_to_current_image(self, assistant_text):
        ...
        if not assistant_text or "xanylabeling_review_candidates" not in assistant_text:
            return 0
    ...
    def add_message(self, role, content, delete_last_message=False):
        ...
        if role == "user":
            if "@image" in content or "<image>" in content:
                # 用户显式用了 @image:把对话绑定/重绑到"当前主界面图片",并带上它
                pattern = r"@image\s*(?=\S|$)"
                content = re.sub(pattern, "<image>", content).strip()
                self.active_image = self.parent().filename
                image = self.active_image
            elif self.active_image:
                # 没有再输 @image,但本对话此前已绑定过图片:自动带上,无需重复输入
                image = self.active_image
    ...
    def on_parent_image_changed(self):
        """主窗口切换了当前图片时调用:让 Chatbot 实时同步到这张新图。
        - 重绑分析对象 active_image 为新图(后台分析的确实是新图);
        - 刷新右侧预览;
        - 载入这张图自己的对话历史(与对话框内导航行为一致)。"""
        try:
            self.active_image = self.parent().filename
            self.update_image_preview()
            QTimer.singleShot(
            ...
            logger.error(f"Error syncing chatbot to new image: {e}")
    ...
    def navigate_image(self, direction="next", index=None):
        ...
        try:
            if self.parent().image_list:
                ...
                # 切换图片后,把对话绑定的图片同步成新图——确保后台真正分析的是切换后的图片,
                # 而不是停留在原来那张。
                self.active_image = self.parent().filename
