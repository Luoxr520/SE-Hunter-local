...
class LabelingWidget(LabelDialog):
    ...
    def __init__(  # noqa: C901
        ...
        web_detection_action = action(
            self.tr("Web 实时检测"),
            self.open_web_detection,
            icon="ultralytics",
            tip=self.tr("在浏览器中查看炫酷 HUD 实时检测界面(真实检测管线)"),
        )
        ...
        utils.add_actions(
            ...
            (
                ...
                web_detection_action,     # ④' Web 实时检测:浏览器炫酷 HUD(真实检测)
            ),
        )
        utils.add_actions(
            self.menus.tool,
    ...
    def open_web_detection(self):
        """启动 Web 实时检测:用真实检测管线推 MJPEG + JSON 状态给浏览器炫酷前端。
        数据集/模型与 Live Detection 同源(QSettings)。仅监听 127.0.0.1。"""
        import webbrowser
        from PyQt6 import QtWidgets as W

        # 1) 选源(摄像头 / 视频 / 图片文件夹)
        dlg = W.QDialog(self)
        dlg.setWindowTitle(self.tr("启动 Web 实时检测"))
        lay = W.QFormLayout(dlg)
        mode_combo = W.QComboBox()
        mode_combo.addItem(self.tr("摄像头"), "camera")
        mode_combo.addItem(self.tr("视频文件"), "video")
        mode_combo.addItem(self.tr("图片文件夹"), "images")
        src_edit = W.QLineEdit("0")
        browse_btn = W.QPushButton(self.tr("浏览…"))
        src_row = W.QHBoxLayout()
        src_row.addWidget(src_edit)
        src_row.addWidget(browse_btn)
        src_wrap = W.QWidget()
        src_wrap.setLayout(src_row)
        port_spin = W.QSpinBox()
        port_spin.setRange(1024, 65535)
        port_spin.setValue(8420)

        # 模型下拉:与 Live Detection 同款候选。data 携带"加载什么":
        # None=注册表当前模型(带热替换);裸名=官方基准(走缓存);路径=注册表某版本 .pt
        model_combo = W.QComboBox()
        model_combo.setMinimumWidth(240)
        model_combo.addItem(self.tr("注册表当前模型(自动热替换)"), None)
        for w in ("yolo11n.pt", "yolo11s.pt", "yolo26n.pt", "yolo26s.pt", "yolov8n.pt"):
            model_combo.addItem(self.tr("基准 ") + w, w)
        # 追加注册表里已训练的版本(读 QSettings 数据集 → registry)
        try:
            _ds0 = ""
            _st = getattr(self, "settings", None)
            if _st is not None:
                _ds0 = str(_st.value("publish/output_dir", "") or "")
            if _ds0:
                from anylabeling.services.auto_training.model_registry import (
                    ModelRegistry,
                )
                import os.path as _osp
                _reg = ModelRegistry(_ds0)
                _mdir = _osp.join(_ds0, "registry", "models")
                for _e in (_reg.list() or []):
                    _mid = _e.get("id")
                    if not _mid:
                        continue
                    _pt = _osp.join(_mdir, f"{_mid}.pt")
                    _lab = self.tr("版本 ") + str(_mid)
                    _mp = (_e.get("metrics", {}) or {}).get("map5095")
                    if isinstance(_mp, (int, float)):
                        _lab += f"  (mAP50-95={_mp:.3f})"
                    model_combo.addItem(_lab, _pt)
        except Exception:
            pass

        def on_mode(_=None):
            src_edit.setText("0" if mode_combo.currentData() == "camera" else "")
        mode_combo.currentIndexChanged.connect(on_mode)

        def on_browse():
            md = mode_combo.currentData()
            if md == "video":
                f, _ = W.QFileDialog.getOpenFileName(
                    dlg, self.tr("选择视频"), "",
                    "Video (*.mp4 *.avi *.mov *.mkv *.flv)")
                if f:
                    src_edit.setText(f)
            elif md == "images":
                d = W.QFileDialog.getExistingDirectory(dlg, self.tr("选择图片文件夹"))
                if d:
                    src_edit.setText(d)
            else:
                W.QMessageBox.information(dlg, self.tr("提示"),
                                          self.tr("摄像头模式请直接填设备号(如 0)"))
        browse_btn.clicked.connect(on_browse)

        lay.addRow(self.tr("来源"), mode_combo)
        lay.addRow(self.tr("源地址/设备号"), src_wrap)
        lay.addRow(self.tr("模型"), model_combo)
        lay.addRow(self.tr("端口"), port_spin)
        btns = W.QDialogButtonBox(
            W.QDialogButtonBox.StandardButton.Ok | W.QDialogButtonBox.StandardButton.Cancel)
        ...
        source = src_edit.text().strip()
        port = port_spin.value()
        model_weight = model_combo.currentData()   # None=注册表当前模型(热替换);否则指定权重
        if mode != "camera" and not source:
            self.error_message(self.tr("缺少来源"), self.tr("请选择视频文件或图片文件夹。"))
            return

        # 2) 数据集 / 模型 与 Live Detection 同源(QSettings)
        dataset = ""
        try:
            st = getattr(self, "settings", None)
            if st is not None:
                dataset = str(st.value("publish/output_dir", "") or "")
        except Exception:
            dataset = ""
        # 只有"注册表当前模型"(model_weight=None)才必须有数据集(要读 registry);
        # 选了具体基准/版本则可不依赖(基准用 COCO 类名,版本是绝对路径)。
        if model_weight is None and not dataset:
            self.error_message(
                self.tr("缺少数据集"),
                self.tr("选择『注册表当前模型』需要先在 Model Registry 设置/选择含模型的数据集;\n"
                        "或在上面的模型下拉里直接选一个基准模型(如 yolo26n)。"))
            return
        if not dataset:
            dataset = "."   # 基准/版本模式:数据集仅占位

        device = "0"   # 与 Live Detection 默认一致;如需 CPU 可后续做成选项
        ...
        try:
            ...
            self.error_message(self.tr("启动失败"), str(e))
            return

        try:
            srv = WebDetectServer(
                dataset=dataset, source=source, mode=mode, device=device,
                conf=0.25, imgsz=640, model_weight=model_weight,
                max_disp_w=1280, dwell_ms=1200, host="127.0.0.1", port=port)
            srv.start()
        ...
        W.QMessageBox.information(
            self, self.tr("Web 实时检测已启动"),
            self.tr("浏览器已打开:") + f"\n{url}\n\n" +
            self.tr("如未自动打开,请手动访问该地址。关闭主程序即停止服务。"))
    ...
    def open_chatbot(self):
        # 复用已打开的实例(避免开多个);非模态显示,这样主窗口仍可切换图片并实时通知它
        dlg = getattr(self, "_chatbot_dialog", None)
        if dlg is not None:
            try:
                dlg.show()
                dlg.raise_()
        ...
        self._chatbot_dialog = dialog   # 存引用,便于主窗口切换图片时通知它刷新
        dialog.setModal(False)          # 非模态:不阻塞主窗口,可一边切图一边看 Chatbot
        # 关闭时清掉引用
        dialog.finished.connect(lambda _=None: setattr(self, "_chatbot_dialog", None))
        dialog.show()
        dialog.raise_()
    ...
    def edit_label(self, item=None):
        ...
        shape.difficult = difficult
        shape.kie_linking = kie_linking
        # 人工确认/修改过标签后,这个框就是人标的,模型置信度不再有意义,清掉,
        # 否则会沿用原 AI 预测框残留的 score(如 0.03)继续显示。
    ...
    def new_shape(self):
        ...
        if text:
            ...
            shape.difficult = difficult
            shape.kie_linking = kie_linking
            # 人工给框定标后清掉模型置信度:手标框本就该没有分数,
            # 若该框源自 AI 预测(带 score),也在此一并清掉,避免显示残留的 0.03 之类。
            if hasattr(shape, "score"):
                shape.score = None
    ...
    def load_file(self, filename=None):  # noqa: C901
        ...
        # 若 Chatbot 正开着,主窗口切换图片后通知它实时同步(刷新预览 + 重绑分析对象)
        dlg = getattr(self, "_chatbot_dialog", None)
        if dlg is not None:
            try:
                dlg.on_parent_image_changed()
            except Exception:
                pass
